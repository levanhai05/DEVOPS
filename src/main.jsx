...
function Login({admin=false}){
  ...
  return <main className="login-page">
    <div className="login-visual">
      ...
      <div className="login-visual-overlay"><span className="eyebrow">{admin ? "LUMA ADMIN" : "LUMA MEMBER"}</span><h1>{admin ? "Quản trị" : "Chào mừng trở lại"}</h1><p>{admin ? "Quản lý cửa hàng, đơn hàng và sản phẩm trong một không gian trực quan." : "Đăng nhập để theo dõi đơn hàng và tận hưởng trải nghiệm mua sắm dành riêng cho bạn."}</p></div>
    </div>
...
function Admin(){
  ...
  return <div className="admin-shell"><aside className="admin-side"><a href="#/" className="logo admin-logo"><span className="logo-mark">H</span>HAI</a><div className="admin-label">QUẢN TRỊ</div>{nav.map(([id,label,Icon])=><button key={id} className={section===id?"admin-nav active":"admin-nav"} onClick={()=>setSection(id)}><Icon size={18}/>{label}</button>)}<div className="side-bottom"><button className="admin-nav"><Settings size={18}/>Cài đặt</button><button className="admin-nav" onClick={()=>{localStorage.removeItem("hai_admin_auth");window.location.href="/admin/login"}}><LogOut size={18}/>Đăng xuất</button></div></aside><main className="admin-main"><div className="admin-top"><div><span className="eyebrow">MONDAY · 11 SEPTEMBER 2026</span><h1>{nav.find(x=>x[0]===section)?.[1]}</h1></div><div className="admin-user"><button className="icon-btn"><Bell size={18}/></button></div></div>{section==="overview"&&<Dashboard inventory={inventory}/>} {section==="orders"&&<Orders/>}{section==="products"&&<Products inventory={inventory} setInventory={setInventory} openAdd={()=>setShowAdd(true)}/>} {section==="customers"&&<Customers/>}{section==="analytics"&&<Analytics/>}</main>{showAdd&&<AddProduct close={()=>setShowAdd(false)} add={p=>{setInventory(x=>[{...p,id:Date.now()},...x]);setShowAdd(false);setNotice("Đã thêm sản phẩm mới")}}/>}{notice&&<div className="toast"><Check size={18}/>{notice}</div>}</div>
}
